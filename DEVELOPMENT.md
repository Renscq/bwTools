...
README.qmd -> README.md render step before site construction. The QMD remains
the only hand-edited README source, while the generated Markdown is consumed by
pkgdown and GitHub. The release preserves the 15-function public API, BwgTrack
schema v2, and all numerical and binary I/O algorithms.
...
- Keep `{r}` for executable R chunks and `{.text}` for non-executable text
  blocks in QMD source files.
