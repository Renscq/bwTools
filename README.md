...
implementation details. Version 0.7.0 adds a reproducible large-BigWig benchmark
suite before performance optimization is attempted.
...
baseline_heap_mb  R heap use immediately before the timed expression
peak_heap_mb      approximate maximum R heap use
heap_delta_mb     approximate additional R heap above baseline
...
`baseline_heap_mb`, `peak_heap_mb`, and `heap_delta_mb` are derived from
`gc(reset = TRUE)` and are **not** operating-system RSS. Likewise, `metadata_cache_cold` clears only the bwTools metadata cache; it
does not flush the operating-system file cache. These limitations are recorded
...
- **0.7.0**: reproducible large-BigWig benchmark suite — current stage.
- **0.7.1**: optimize only bottlenecks demonstrated by 0.7.0 benchmark results.
