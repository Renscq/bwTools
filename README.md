...
Current development release: **0.7.3**
...
0.7.2 refined indexed retrieval and memory diagnostics, and version 0.7.3 adds
real native-writer profiling without changing the public analysis API.
...
*before* writer timing, then benchmarks `write_bwg()` with both `zoom = FALSE`
and the default `zoom = TRUE`. Indexed retrieval is therefore excluded from the
writer timing itself. Writer cases larger than `write_max_bases` are recorded as
skipped.
...
For the first real writer benchmark, a focused configuration is recommended:
...
- **0.7.4**: conditional writer optimization only if the real benchmark identifies a bottleneck.
